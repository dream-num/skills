...
- [`benchmarking-univer-cli`](./skills/benchmarking-univer-cli/SKILL.md): 面向 prepared SaC workspace 的 SpreadsheetBench solver protocol
...
- `benchmarking-univer-cli` 负责 `/task` 内非交互 SpreadsheetBench solver agent 的协议，约束 prepared SaC workspace、`answer_position` 和 `output.xlsx` 交付
...
Benchmark solver prompt 直接使用 `benchmarking-univer-cli`，再由它进入 Univer skill stack。
...
| [`benchmarking-univer-cli`](./skills/benchmarking-univer-cli/SKILL.md) | SpreadsheetBench solver protocol for `/task`, prepared SaC workspaces, evaluator windows, and `output.xlsx` handoff | 必须停留在 SaC 且避免 hidden-answer 泄漏的 benchmark solver agents | canonical |
