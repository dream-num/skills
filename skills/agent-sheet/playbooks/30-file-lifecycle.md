...
### Flush local entry state

Use this when a local entry already exists but another tool requires a clean persisted snapshot.
...
- explicit persist fails for a local entry that should be recoverable
