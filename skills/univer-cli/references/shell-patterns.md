...
```bash
univer pipe out <univer-path> --range "Claims!A1:H200000" --format tsv \
  | awk -F'\t' 'BEGIN{OFS="\t"} NR==1 || $5=="P1"{print $1,$2,$5}' \
  | univer pipe in <univer-path> --range "ClaimsP1Review!A1:C200000" --input-format tsv --data-stdin
```
...
```bash
univer pipe out <univer-path> --range "Claims!A1:H200000" --type rawValue --format tsv \
...
```bash
univer pipe out <univer-path> --range 'ApprovalQueue!A1:G5' --format csv \
...
```bash
univer pipe out <univer-path> --range '工作表1!A1:J3' --format csv \
