...
Doc table dimensions use layout pixels (`px`). A default medium modern Doc has about `826.67 px` of
usable body width. Tables created without an explicit total width use `100 px` for each unspecified
column. A full-width table should use the available body width; when column widths are specified,
their total should match the table width.
