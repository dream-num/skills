// Managed Univer inspect tool: sheet-overview
// Run with: univer inspect <file.univer> --tool sheet-overview --params <params.json|->
...
async function inspectSheetOverviewTool({ params, context, univerAPI }) {
  ...
  const warnings = [];
  if (sheets.length > selectedSheets.length) {
    warnings.push("Only the first " + selectedSheets.length + " of " + sheets.length + " sheets were returned.");
  }
  ...
  return envelope("sheet-overview", {
    ...
    sheets: evidence
  }, warnings.concat(evidence.flatMap((entry) => entry.warnings)), sheets.length > selectedSheets.length || evidence.some((entry) => entry.truncated));
}
return await inspectSheetOverviewTool({ params, context, univerAPI });
function readSheetOverview(sheet, maxRows, maxColumns) {
  ...
  const sampleBounds = {
    ...
    endRow: Math.min(used.endRow, used.startRow + maxRows - 1),
    endColumn: Math.min(used.endColumn, used.startColumn + maxColumns - 1)
  };
  const sampleRangeA1 = boundsToA1(sampleBounds);
  if (sampleBounds.endRow < used.endRow || sampleBounds.endColumn < used.endColumn) {
    warnings.push("Sample is truncated from used range " + boundsToA1(used) + ".");
  }
  const tailBounds = {
    startRow: Math.max(used.startRow, used.endRow - maxRows + 1),
    startColumn: Math.max(used.startColumn, used.endColumn - maxColumns + 1),
    endRow: used.endRow,
    endColumn: used.endColumn
  };
  const headSample = readOverviewSample(sheet, sampleBounds);
  const tailSample = readOverviewSample(sheet, tailBounds);
  const regions = readSheetRegions(sheet, used, maxRows, maxColumns, warnings);
  const formulaCellsInSamples = uniqueFormulaCells([
    ...listSampleFormulaCells(headSample),
    ...listSampleFormulaCells(tailSample),
    ...regions.flatMap((entry) => readRegionSamples(entry).flatMap((sample) => listSampleFormulaCells(sample)))
  ]);
  const regionSamples = regions.flatMap((entry) => readRegionSamples(entry));
  const result = {
    sheetName,
    usedRangeA1: boundsToA1(used),
    rowCount: used.rowCount,
    columnCount: used.columnCount,
    sample: headSample,
    regions,
    formulaCellCountInSamples: formulaCellsInSamples.length,
    formulaCellsInSamples,
    nonEmptyRowBandsInSamples: compactNumberBands([
      ...collectNonEmptyRows(headSample.values, sampleBounds),
      ...collectNonEmptyRows(tailSample.values, tailBounds),
      ...regionSamples.flatMap((sample) => collectNonEmptyRows(sample.values, parseRangeA1(sample.rangeA1)))
    ]),
    nonEmptyColumnBandsInSamples: compactNumberBands([
      ...collectNonEmptyColumns(headSample.values, sampleBounds),
      ...collectNonEmptyColumns(tailSample.values, tailBounds),
      ...regionSamples.flatMap((sample) => collectNonEmptyColumns(sample.values, parseRangeA1(sample.rangeA1)))
    ]),
    warnings,
    truncated: warnings.length > 0
  };
  if (!samplesEquivalent(headSample, tailSample)) {
    result.samples = {
      head: headSample,
      tail: tailSample
    };
...
function readSheetRegions(sheet, used, maxRows, maxColumns, warnings) {
  const maxRegionScanCells = 100000;
  const cellCount = used.rowCount * used.columnCount;
  if (cellCount > maxRegionScanCells) {
    warnings.push("Region evidence skipped because used range " + boundsToA1(used) + " contains " + cellCount + " cells, above scan limit " + maxRegionScanCells + ".");
    return [];
  }
  const matrix = readRangePayload(sheet, boundsToA1(used), ["normalizedValues"]).normalizedValues ?? [];
  const columnBands = compactNumberBands(collectNonEmptyColumns(matrix, used));
  const regions = [];
  for (const columnBand of columnBands) {
    const rowBands = compactNumberBands(collectNonEmptyRowsInColumnBand(matrix, used, columnBand));
    for (const rowBand of rowBands) {
      const bounds = makeBounds(rowBand.start, columnBand.start, rowBand.end, columnBand.end);
      regions.push(readRegionEvidence(sheet, bounds, maxRows, maxColumns, regions.length + 1));
    }
  }
  return regions;
}
function collectNonEmptyRowsInColumnBand(matrix, bounds, columnBand) {
  const rows = [];
  ...
  for (let rowIndex = 0; rowIndex < matrix.length; rowIndex += 1) {
    ...
    for (let column = columnBand.start; column <= columnBand.end; column += 1) {
      if (isNonEmptyValue(row[column - bounds.startColumn])) {
        rows.push(bounds.startRow + rowIndex);
        break;
      }
    }
  }
...
function readRegionEvidence(sheet, bounds, maxRows, maxColumns, index) {
  const leftStartColumn = bounds.startColumn;
  const leftEndColumn = Math.min(bounds.endColumn, bounds.startColumn + maxColumns - 1);
  const headBounds = makeBounds(
    bounds.startRow,
    leftStartColumn,
    Math.min(bounds.endRow, bounds.startRow + maxRows - 1),
    leftEndColumn
  );
  const tailBounds = makeBounds(
    Math.max(bounds.startRow, bounds.endRow - maxRows + 1),
    leftStartColumn,
    bounds.endRow,
    leftEndColumn
  );
  const samples = {
    head: readOverviewSample(sheet, headBounds),
    tail: readOverviewSample(sheet, tailBounds)
  };
  if (bounds.columnCount > maxColumns) {
    const rightStartColumn = Math.max(bounds.startColumn, bounds.endColumn - maxColumns + 1);
    const rightEndColumn = bounds.endColumn;
    samples.rightHead = readOverviewSample(sheet, makeBounds(
      bounds.startRow,
      rightStartColumn,
      Math.min(bounds.endRow, bounds.startRow + maxRows - 1),
      rightEndColumn
    ));
    samples.rightTail = readOverviewSample(sheet, makeBounds(
      Math.max(bounds.startRow, bounds.endRow - maxRows + 1),
      rightStartColumn,
      bounds.endRow,
      rightEndColumn
  ...
  const result = {
    ...
    columns: {
      start: bounds.startColumn,
      end: bounds.endColumn,
      startA1: columnIndexToName(bounds.startColumn),
      endA1: columnIndexToName(bounds.endColumn)
    },
    truncated: bounds.rowCount > maxRows || bounds.columnCount > maxColumns
  };
  if (
    samples.rightHead === undefined &&
    samples.rightTail === undefined &&
    samplesEquivalent(samples.head, samples.tail)
  ) {
...
function makeBounds(startRow, startColumn, endRow, endColumn) {
  return {
    startRow,
    startColumn,
    endRow,
...
function readOverviewSample(sheet, bounds) {
  const rangeA1 = boundsToA1(bounds);
  const payload = readRangePayload(sheet, rangeA1, ["normalizedValues", "formulas"]);
  const sample = {
    rangeA1,
    values: payload.normalizedValues
  };
  const formulaCells = listFormulaCells(payload.formulas, bounds, Number.POSITIVE_INFINITY);
  if (formulaCells.length > 0) {
    sample.formulaCells = formulaCells;
  }
  return sample;
}
function listSampleFormulaCells(sample) {
  return Array.isArray(sample.formulaCells) ? sample.formulaCells : [];
}
function readRegionSamples(region) {
  if (region.sample !== undefined) {
    return [region.sample];
  }
  return Object.values(region.samples ?? {});
}
function samplesEquivalent(left, right) {
  return JSON.stringify(left) === JSON.stringify(right);
}
function collectNonEmptyRows(matrix, bounds) {
  const rows = [];
  if (!Array.isArray(matrix)) {
    return rows;
  }
  for (let rowIndex = 0; rowIndex < matrix.length; rowIndex += 1) {
    const row = Array.isArray(matrix[rowIndex]) ? matrix[rowIndex] : [];
    if (row.some((value) => isNonEmptyValue(value))) {
      rows.push(bounds.startRow + rowIndex);
    }
  }
  return rows;
}
function collectNonEmptyColumns(matrix, bounds) {
  const columns = [];
  if (!Array.isArray(matrix)) {
    return columns;
  }
  const maxColumnCount = Math.max(...matrix.map((row) => Array.isArray(row) ? row.length : 0), 0);
  for (let columnIndex = 0; columnIndex < maxColumnCount; columnIndex += 1) {
    if (matrix.some((row) => Array.isArray(row) && isNonEmptyValue(row[columnIndex]))) {
      columns.push(bounds.startColumn + columnIndex);
    }
  }
  return columns;
}
function compactNumberBands(values) {
  const sorted = [...new Set(values)].sort((left, right) => left - right);
  const bands = [];
  for (const value of sorted) {
    const last = bands[bands.length - 1];
    if (last != null && last.end + 1 === value) {
      last.end = value;
      continue;
    }
    bands.push({ start: value, end: value });
  }
  return bands;
}
function uniqueFormulaCells(cells) {
  const seen = new Set();
  const unique = [];
  for (const cell of cells) {
    const key = cell.a1 + ":" + cell.formula;
    if (seen.has(key)) {
      continue;
    }
    seen.add(key);
