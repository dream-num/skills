...
function readSheetOverview(sheet, maxRows, maxColumns) {
  ...
  const formulaCellsInSamples = uniqueFormulaCells([
    ...listFormulaCells(headSample.formulas, sampleBounds, Number.POSITIVE_INFINITY),
    ...listFormulaCells(tailSample.formulas, tailBounds, Number.POSITIVE_INFINITY),
    ...regions.flatMap((entry) => Object.values(entry.samples).flatMap((sample) => listFormulaCells(sample.formulas, parseRangeA1(sample.rangeA1), Number.POSITIVE_INFINITY)))
  ]);
  const regionSamples = regions.flatMap((entry) => Object.values(entry.samples));
  return {
    sheetName,
    ...
    nonEmptyRowBandsInSamples: compactNumberBands([
      ...collectNonEmptyRows(headSample.normalizedValues, sampleBounds),
      ...collectNonEmptyRows(tailSample.normalizedValues, tailBounds),
      ...regionSamples.flatMap((sample) => collectNonEmptyRows(sample.normalizedValues, parseRangeA1(sample.rangeA1)))
    ]),
    nonEmptyColumnBandsInSamples: compactNumberBands([
      ...collectNonEmptyColumns(headSample.normalizedValues, sampleBounds),
      ...collectNonEmptyColumns(tailSample.normalizedValues, tailBounds),
      ...regionSamples.flatMap((sample) => collectNonEmptyColumns(sample.normalizedValues, parseRangeA1(sample.rangeA1)))
    ]),
    warnings,
