// Managed Univer inspect tool: sheet-conditional-formats
// Run with: univer inspect <file.univer> --tool sheet-conditional-formats --params <params.json|->
const __univerManagedInspectTool = true;

async function inspectSheetConditionalFormatsTool({ params, context, univerAPI }) {
  params = requireObjectParams(params);
  const unitId = requireString(params, "unitId");
  const sheetName = requireString(params, "sheetName");
  const requestedRangeA1 = typeof params.rangeA1 === "string" ? normalizeRangeA1(params.rangeA1) : null;
  const requestedBounds = requestedRangeA1 === null ? null : parseRangeA1(requestedRangeA1);
  const maxRules = readLimit(params.maxRules, 200, 1, 1000);
  ...
  if (typeof sheet.getConditionalFormattingRules !== "function") {
    return envelope("sheet-conditional-formats", {
      unitId,
      unitType: unit.type,
      sheetName,
      ...(requestedRangeA1 === null ? {} : { rangeA1: requestedRangeA1 })
    }, {
      rules: [],
      ruleCount: 0
    }, ["Sheet facade does not expose getConditionalFormattingRules()."], false);
  }
  const warnings = [];
  const runtimeRules = sheet.getConditionalFormattingRules();
  const normalized = [];
  runtimeRules.forEach((rule, index) => {
    const entry = normalizeConditionalFormattingRule(rule, index, requestedBounds, warnings);
    if (entry !== null) {
      normalized.push(entry);
    }
  });
  const limited = normalized.slice(0, maxRules);
  const truncated = normalized.length > limited.length;
  if (truncated) {
    warnings.push("Only the first " + limited.length + " of " + normalized.length + " matching conditional formatting rules were returned.");
  }
  return envelope("sheet-conditional-formats", {
    unitId,
    unitType: unit.type,
...
return await inspectSheetConditionalFormatsTool({ params, context, univerAPI });
