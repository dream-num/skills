// Managed Univer inspect tool: sheet-range
// Run with: univer inspect <file.univer> --tool sheet-range --params <params.json>
...
async function inspectSheetRangeTool({ params, context, univerAPI }) {
  ...
  const totalCells = rangeRequests.reduce((sum, request) => sum + countRangeCells(request.rangeA1), 0);
  if (totalCells > maxCells) {
    throw new Error("sheet-range requested " + totalCells + " cells, exceeding maxCells " + maxCells + ".");
  }
  const ranges = rangeRequests.map((request) => {
    const sheet = getSheet(workbook, request.sheetName);
    const rangePayload = readRangePayload(sheet, request.rangeA1, include, styleTraits);
    return {
      ...(request.label === undefined ? {} : { label: request.label }),
      sheetName: request.sheetName,
      rangeA1: rangePayload.rangeA1,
      rowCount: rangePayload.rowCount,
      columnCount: rangePayload.columnCount,
      cells: readCellFacts(rangePayload),
      ...rangePayload
    };
  });
  const firstRange = ranges[0];
  const payload = ranges.length === 1 && params.ranges === undefined
    ? { ...firstRange, ranges }
    : { ranges };
...
return await inspectSheetRangeTool({ params, context, univerAPI });
