// Managed Univer inspect tool: sheet-search
// Run with: univer inspect <file.univer> --tool sheet-search --params <params.json>
...
async function inspectSheetSearchTool({ params, context, univerAPI }) {
  ...
  for (const sheet of sheets) {
    ...
    const bounds = typeof params.rangeA1 === "string" ? parseRangeA1(params.rangeA1) : getUsedBounds(sheet);
    if (bounds.rowCount === 0 || bounds.columnCount === 0) {
      continue;
    }
    const rangeA1 = boundsToA1(bounds);
    const payload = readRangePayload(sheet, rangeA1, types);
    collectMatches({
      matches,
      maxResults,
      query,
    ...
    if (matches.length >= maxResults) {
      warnings.push("Search reached maxResults " + maxResults + ".");
      break;
    }
  }
...
return await inspectSheetSearchTool({ params, context, univerAPI });
function collectMatches(input) {
  const queryText = String(input.query);
  for (const type of input.types) {
    const matrix = input.payload[type];
    ...
    for (let rowIndex = 0; rowIndex < matrix.length; rowIndex += 1) {
      ...
      for (let columnIndex = 0; columnIndex < row.length; columnIndex += 1) {
        ...
        if (!matchesValue(value, queryText, input.match)) {
          continue;
        }
        const a1 = cellToA1(input.bounds.startRow + rowIndex, input.bounds.startColumn + columnIndex);
        const match = {
          sheetName: input.sheetName,
          a1,
          row: input.bounds.startRow + rowIndex,
          column: input.bounds.startColumn + columnIndex,
          matchedType: type,
          value
        };
        if (input.neighborhood > 0) {
          const around = expandBounds({ startRow: input.bounds.startRow + rowIndex, startColumn: input.bounds.startColumn + columnIndex, endRow: input.bounds.startRow + rowIndex, endColumn: input.bounds.startColumn + columnIndex }, input.neighborhood, input.neighborhood, input.neighborhood, input.neighborhood, getUsedBounds(input.sheet));
          match.neighborhood = {
            rangeA1: boundsToA1(around),
            normalizedValues: readRangePayload(input.sheet, boundsToA1(around), ["normalizedValues"]).normalizedValues
          };
        }
...
function matchesValue(value, query, match) {
