// Managed Univer inspect tool: sheet-formulas
// Run with: univer inspect <file.univer> --tool sheet-formulas --params <params.json>
...
async function inspectSheetFormulasTool({ params, context, univerAPI }) {
  ...
  for (const sheet of sheets) {
    ...
    const payload = readRangePayload(sheet, boundsToA1(bounds), ["displayValues", "rawValues", "formulas"]);
    for (const cell of listFormulaCells(payload.formulas, bounds, maxResults - formulas.length)) {
      const entry = {
        sheetName,
        a1: cell.a1,
        row: cell.row,
        ...
        displayValue: readMatrixCell(payload.displayValues, cell.row - bounds.startRow, cell.column - bounds.startColumn),
        rawValue: readMatrixCell(payload.rawValues, cell.row - bounds.startRow, cell.column - bounds.startColumn)
      };
      if (includeNeighborLabels) {
        entry.neighborLabels = readNeighborLabels(sheet, cell.row, cell.column);
      }
      formulas.push(entry);
      if (formulas.length >= maxResults) {
        warnings.push("Formula scan reached maxResults " + maxResults + ".");
        break;
      }
    }
    if (formulas.length >= maxResults) {
...
return await inspectSheetFormulasTool({ params, context, univerAPI });
function readNeighborLabels(sheet, row, column) {
  const labels = {};
  if (row > 0) {
    labels.above = readRangePayload(sheet, cellToA1(row - 1, column), ["displayValues"]).displayValues[0]?.[0] ?? null;
  }
  if (column > 0) {
    labels.left = readRangePayload(sheet, cellToA1(row, column - 1), ["displayValues"]).displayValues[0]?.[0] ?? null;
  }
  return labels;
}
