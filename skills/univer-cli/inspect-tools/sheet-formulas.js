...
async function inspectSheetFormulasTool({ params, context, univerAPI }) {
  ...
  for (const sheet of sheets) {
    ...
    const payload = readRangePayload(sheet, boundsToA1(bounds), ["values", "displayValues", "formulas"]);
    for (const cell of listFormulaCells(payload.formulas, bounds, maxResults - formulas.length)) {
      const entry = {
        ...
        value: readMatrixCell(payload.values, cell.row - bounds.startRow, cell.column - bounds.startColumn)
      };
