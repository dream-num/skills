...
Prefer content search when it returns real matches in the installed CLI:

```bash
univer search "$WB" West
```

If search is unavailable or insufficient, inspect a bounded range and derive the edit boundary from visible headers and sample rows:
